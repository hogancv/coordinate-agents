...
test('runs the complete cross-platform message lifecycle idempotently', () => {
  ...
  try {
    ...
    const waiting = invoke(repo, ['wait', '--agent', 'antigravity', '--timeout-minutes', '1', '--poll-seconds', '0.01']);
    ...
    assert.equal(invoke(repo, ['state', '--agent', 'antigravity', '--state', 'WAITING']).status, 0);
    assert.equal(invoke(repo, ['state', '--agent', 'antigravity', '--state', 'APPROVED']).status, 0);
...
test('supports two concurrent waiting processes without cross-claiming', async () => {
  const repo = repository();
  try {
    const codexWait = runAsync(repo, ['wait', '--agent', 'codex', '--timeout-minutes', '0.1', '--poll-seconds', '0.01']);
    const agyWait = runAsync(repo, ['wait', '--agent', 'antigravity', '--timeout-minutes', '0.1', '--poll-seconds', '0.01']);
...
test('recovers interrupted claims and stale locks', () => {
  ...
  try {
    ...
    const claimed = invoke(repo, ['wait', '--agent', 'antigravity', '--timeout-minutes', '1', '--poll-seconds', '0.01']).stdout.trim();
    ...
    const recovered = invoke(repo, ['recover', '--agent', 'antigravity', '--stale-after-seconds', '1']);
    ...
    const reclaimed = invoke(repo, ['wait', '--agent', 'antigravity', '--timeout-minutes', '1', '--poll-seconds', '0.01']);
...
test('uses the recorded lease expiry rather than the recovery fallback age', async () => {
  ...
  try {
    ...
    const claimed = invoke(repo, ['wait', '--agent', 'antigravity', '--timeout-minutes', '1', '--poll-seconds', '0.01', '--lease-seconds', '0.1']).stdout.trim();
    await new Promise(resolvePromise => setTimeout(resolvePromise, 150));
    const recovered = JSON.parse(invoke(repo, ['recover', '--agent', 'antigravity', '--stale-after-seconds', '999']).stdout);
...
test('times out cleanly and quarantines damaged messages', () => {
  const repo = repository();
  try {
    const timeout = invoke(repo, ['wait', '--agent', 'codex', '--timeout-minutes', '0.002', '--poll-seconds', '0.01']);
    ...
    const wait = invoke(repo, ['wait', '--agent', 'codex', '--timeout-minutes', '1', '--poll-seconds', '0.01']);
...
test('falls back from a damaged newest state record', () => {
  const repo = repository();
  try {
    invoke(repo, ['state', '--agent', 'codex', '--state', 'WAITING', '--details', 'valid']);
...
test('requires --agent option when waiting or changing state', () => {
  const repo = repository();
  try {
    const result = invoke(repo, ['wait']);
    assert.equal(result.status, 1);
    assert.match(result.stderr, /--agent is required for wait/);
  } finally {
    rmSync(repo, { recursive: true, force: true });
  }
});
