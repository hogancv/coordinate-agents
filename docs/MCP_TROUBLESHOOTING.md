...
Plugin is the intended `2.3.0` payload and inspect its actual root, not only the
marketplace entry:
...
still report version `2.3.0` while pointing at an older Git revision that does
...
codex plugin marketplace add hogancv/coordinate-agents --ref v2.3.0
