...
import { buildAgentPrompt } from '../skills/coordinate-agents/scripts/role-prompts.mjs';
...
async function startLocalUiServer(options, kind) {
  const { startInspector, startWorkspace } = await import('../inspector/server/server.mjs');
  const root = kind === 'workspace' ? resolve(options.root || process.cwd()) : assertGitRepository(options.root, messages.en);
  // The Web Workspace is not an Inspector skin: it validates and binds a real
  // Git root, mounts the guarded Action Gateway, and injects the per-launch
  // capability into the served HTML. The Inspector compatibility surface stays
  // strictly read-only (GET only, no capability, no gateway).
  const isWorkspace = kind === 'workspace';
  const started = isWorkspace
    ? await startWorkspace({ root, port: options.port || 3000 })
    : await startInspector({ root, port: options.port || 3000 });
  if (options.json) {
...
function schedulingDecision(graph, subtask, agent, eligibleIds, capacityLimitedIds, conflictById, waveReasons, intentCoverageAvailable, subtasksById = null) {
  // Performance optimization: use Map for O(1) dependency lookup instead of O(N) Array.find
  const map = subtasksById || new Map((graph.subtasks || []).map(candidate => [candidate.id, candidate]));
...
async function taskGraphPlanCommand(options, { json = false } = {}) {
  ...
  // Performance optimization: build a subtask Map once for O(1) dependency lookups across all subtasks
